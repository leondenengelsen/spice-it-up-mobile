...
const { getUserAllergies, getUserAllergiesFiltered, getUserLowFodmap, getUserVegetarian, getUserAdventurenessValue } = require('./userOptionsController');
...
exports.generateSuggestions = async (req, res) => {
  try {
    ...
    const allergies = await getUserAllergiesFiltered(userId); // Use filtered version (excludes lowfodmap and vegetarian)
    const isLowFodmap = await getUserLowFodmap(userId); // Check Low FODMAP separately
    const isVegetarian = await getUserVegetarian(userId); // Check vegetarian separately
    ...
    console.log("   - Vegetarian diet:", isVegetarian);
    ...
    // Get the complete system message with user preferences (now includes Low FODMAP and vegetarian separately)
    const systemMessage = getSystemMessage(mode, portions, isDetailedRecipe, adventurousness, allergies, isLowFodmap, isVegetarian, recipeIdea);
